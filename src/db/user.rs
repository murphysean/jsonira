...
impl UserDb {
    ...
    #[tracing::instrument(level = "info")]
    pub async fn create_user(&self, email: String, password: String, user: User) -> Result<User, UserDbError> {
        ...
        let connection = self.connection.lock().await;
        static QUERY: &str = "INSERT INTO users (email, password, obj) VALUES (:email,:password,:obj) RETURNING id;";
        let mut statement = connection.prepare(QUERY)?;
        let bstr = to_string(&user)?;
        statement.bind((":email", user.email.as_str()))?;
    ...
    pub async fn update_user(&self, id: i64, mut user: User) -> Result<User, UserDbError> {
        ...
        let connection = self.connection.lock().await;
        static QUERY: &str = "UPDATE users SET email = ?, obj = ? WHERE id = ?;";
        let mut statement = connection.prepare(QUERY)?;
        statement.bind((1, user.email.as_str()))?;
        statement.bind((2, obj.as_str()))?;
        statement.bind((3, id))?;
...
mod tests{
    ...
    fn simple_argon2(){
        ...
        let pwh = Argon2::default().hash_password(password.as_bytes(), &salt).unwrap();
        println!("Result: {}", &pwh);
        let pwhs = pwh.serialize();
